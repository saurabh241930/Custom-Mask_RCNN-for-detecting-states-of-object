...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    for i in range(N):
        ...
        ax.text(x1, y1 + 10, caption,
                color='b', size=14, backgroundcolor="w")

        # Mask
        mask = masks[:, :, i]
        if show_mask:
            if class_id == 1: 
                masked_image = apply_mask(masked_image, mask, [1, 0.84, 0.61], alpha=0.7)
            elif class_id == 2:
                masked_image = apply_mask(masked_image, mask, [1, 0.58, 0.47], alpha=0.7)    
            elif class_id == 3:
                masked_image = apply_mask(masked_image, mask, [0.32,0.69,0.66], alpha=0.7)
            elif class_id == 4:
                masked_image = apply_mask(masked_image, mask, [0.39, 0.59, 0.69], alpha=0.7)
            else:
                masked_image = apply_mask(masked_image, mask, [0.29, 0.9, 0.39], alpha=0.7)
        ...
        for verts in contours:
            ...
            p = Polygon(verts, facecolor="none", edgecolor='w')
            ax.add_patch(p)
